...
window.cancelStarAnimation = function () {
  cancelAnimationFrame(_animFrame);
  clearTimeout(_goingOutTimer);
};
...
window.initStarAnimation = function () {
  ...
  _goingOutTimer = setTimeout(() => {
    goingOut = true;
    killStars();
  }, 1000);

  if (_resizeHandler) window.removeEventListener('resize', _resizeHandler);
  _resizeHandler = () => { resize(); if (!goingOut) initStars(); };
  window.addEventListener('resize', _resizeHandler);
};

// Start when site signals it's ready (after music modal resolved or pref loaded)
document.addEventListener('covaya:ready', window.initStarAnimation);
